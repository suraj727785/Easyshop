...
$placed_shop_id=$row['order_shop_id'];
$order_price=$row['order_rate'];
$order_purchase_price=$row['order_purchase_rate'];
$order_quantity=$row['order_quantity'];
$order_date= date('d-m-y');

$query= " INSERT INTO checkout(username,order_user_id,placed_product_id,placed_shop_id,full_name,mob_no,order_name,order_rate,order_purchase_rate,order_quantity,order_date,full_address) ";

$query.= " VALUES('{$_SESSION['username']}','{$placed_user_id}','{$placed_product_id}','{$placed_shop_id}','{$new_name}','{$new_mobile_no}','{$placed_item}','{$order_price}','{$order_purchase_price}','{$order_quantity}', now() ,'{$full_address}' ) ";
